...
const artifacts = Object.fromEntries(['en', 'ro'].map((locale) => [
  locale,
  JSON.parse(readFileSync(new URL(`../src/content/generated/proof/readings.${locale}.json`, import.meta.url))),
]));
...
test('proof contains the expected reading set in each language', () => {
  assert.equal(report.translationCount, 22);
  for (const locale of ['en', 'ro']) {
    assert.equal(artifacts[locale].locale, locale);
    assert.equal(artifacts[locale].readings.length, 11);
    assert.deepEqual(artifacts[locale].readings.map((reading) => reading.id), [1, 19, 58, 59, 60, 97, 100, 180, 237, 293, 366]);
    for (const reading of artifacts[locale].readings) {
      assert.match(reading.monthDay, /^\d{2}-\d{2}$/);
      assert.ok(reading.title.length > 0);
      assert.ok(reading.plainText.length > 0);
